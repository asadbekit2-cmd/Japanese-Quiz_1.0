...
(function () {
  ...
  const TYPE_UZ = {
    kanji_reading: "Kanji o'qilishi", orthography: "Yozuv (kanji)", context: "Kontekst (affiks)",
    paraphrase: "Kontekst (so'z)", usage: "Sinonim", usage_special: "Qo'llanish", grammar_form: "Grammatik shakl",
    sentence_order: "Gap tuzish", text_grammar: "Matn grammatikasi"
  };

  /* ======= EXAM MODE metadata ======= */
  const EXAM_PROB = {
    kanji_reading:  { no:"問題1", name:"漢字読み",      bigSec:"文字・語彙", color:"vocab",   count:5  },
    orthography:    { no:"問題2", name:"表記",          bigSec:"文字・語彙", color:"vocab",   count:5  },
    context:        { no:"問題3", name:"文脈規定",      bigSec:"文字・語彙", color:"vocab",   count:5 },
    paraphrase:     { no:"問題4", name:"文脈規定",      bigSec:"文字・語彙", color:"vocab",   count:7  },
    usage:          { no:"問題5", name:"言い換え類義",   bigSec:"文字・語彙", color:"vocab",   count:5  },
    usage_special:  { no:"問題6", name:"用法",          bigSec:"文字・語彙", color:"vocab",   count:5  },
    grammar_form:   { no:"問題7", name:"文法形式の判断", bigSec:"文法",      color:"grammar", count:12 },
    sentence_order: { no:"問題8", name:"文の組み立て",  bigSec:"文法",      color:"grammar", count:5  },
    text_grammar:   { no:"問題9", name:"文章の文法",    bigSec:"文法",      color:"grammar", count:5  }
  };

  /* ---------- saqlash ---------- */
  const SKEY = "jlpt_n2_game_v1";
  ...
  function renderHome() {
    ...
    META.levels.forEach(lv => {
      ...
      // Tur tugmalari (9 ta) — qisqa belgilar
      const TYPE_CHIPS = [
        { type:"kanji_reading",  label:"漢", cls:"voc"  },
        { type:"orthography",    label:"表", cls:"voc"  },
        { type:"context",        label:"接", cls:"voc"  },
        { type:"paraphrase",     label:"文脈", cls:"voc"  },
        { type:"usage",          label:"類", cls:"voc"  },
        { type:"usage_special",  label:"用", cls:"voc"  },
  ...
  function startLevelSection(levelId, section) {
    const VOCAB_TYPES = ["kanji_reading","orthography","context","paraphrase","usage","usage_special"];
    ...
    Q = {
      ...
      firstCorrect: 0, hearts: 5, combo: 0, maxCombo: 0,
      startedAt: Date.now(),
      timeLeft: Math.ceil(items.length / 54 * (t.minutes || 45) * 60),
      answered: 0
    };
    show("screen-quiz");
    startTimer();
  ...
  /* ===== LEVEL OPTIONS SHEET ===== */
  const LSO_TYPES = [
    { type:"kanji_reading",  no:"問題1", name:"漢字読み",      sec:"vocab",   emoji:"📖", count:5  },
    { type:"orthography",    no:"問題2", name:"表記",          sec:"vocab",   emoji:"✏️", count:5  },
    { type:"context",        no:"問題3", name:"文脈規定",      sec:"vocab",   emoji:"💬", count:5  },
    { type:"paraphrase",     no:"問題4", name:"文脈規定",      sec:"vocab",   emoji:"🔄", count:7  },
    { type:"usage",          no:"問題5", name:"言い換え類義",   sec:"vocab",   emoji:"🎯", count:5  },
    { type:"usage_special",  no:"問題6", name:"用法",          sec:"vocab",   emoji:"✨", count:5  },
    { type:"grammar_form",   no:"問題7", name:"文法形式",      sec:"grammar", emoji:"📝", count:12 },
    { type:"sentence_order", no:"問題8", name:"文の組み立て",  sec:"grammar", emoji:"🔀", count:5  },
    { type:"text_grammar",   no:"問題9", name:"文章の文法",    sec:"grammar", emoji:"📄", count:5  }
  ];
  ...
  function showLevelOptions(levelId) {
    ...
    $("#lso-total").textContent = "54 savol";
  ...
  function startRandomMock() {
    ...
    // Real JLPT N2 tartibi va soni (jami 54 ta)
    const ORDER = [
      { type: "kanji_reading",  count: 5,  sec: "文字・語彙" },
      { type: "orthography",    count: 5,  sec: "文字・語彙" },
      { type: "context",        count: 5,  sec: "文字・語彙" },
      { type: "paraphrase",     count: 7,  sec: "文字・語彙" },
      { type: "usage",          count: 5,  sec: "文字・語彙" },
      { type: "usage_special",  count: 5,  sec: "文字・語彙" },
      { type: "grammar_form",   count: 12, sec: "文法" },
    ...
    Q = {
      ...
      startedAt: Date.now(), timeLeft: 45 * 60, answered: 0,
